...
class Save extends ControllerBackend {
		...
		if($_SERVER['REQUEST_METHOD'] === 'POST'){
			try{
				...
				if(isset($_FILES['avatar']['name']) && $_FILES['avatar']['name'] != ''){
					...
					$attachment_id = media_handle_upload( 'avatar', null, array() );
				...
				if(!$model->getId()) {
					$this->helper('session')->setData('lecturer_post', $_POST);
					$this->setNotice3('Error when save'); //message
				}else{
					$this->helper('session')->clear('lecturer_post');
				}

			}catch(\Exception $e){
				throw new \Exception('Error when saving Lecturer data '. $e->getMessage());
			}

			$this->_redirect('admin.php?page=soict-lecturer-edit&id='.$model->getId()); //forward controller
		}else{
	...
	public function getModel(){
		...
		return \SoictApp::getModel('lecturer');
	}
