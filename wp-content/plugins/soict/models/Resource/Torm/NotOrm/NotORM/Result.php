...
class NotORM_Result extends NotORM_Abstract implements Iterator, ArrayAccess, Countable, JsonSerializable {
	...
	*/
	...
	protected function query($query, $parameters) {
		if ($this->notORM->isDebug()) {
			if (!is_callable($this->notORM->isDebug())) {
				$debug = "$query;";
				if ($parameters) {
			...
			} elseif (call_user_func($this->notORM->isDebug(), $query, $parameters) === false) {
				return false;
			}
		}
